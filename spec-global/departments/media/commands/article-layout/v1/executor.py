...
2. **标题样式与编号（重要！）**：
   - **一级标题（H1）**：统一使用阿拉伯数字编号，格式为 `<h1>1. 标题</h1>`、`<h1>2. 标题</h1>`
   - **二级标题（H2）**：如果原文有编号，转换为阿拉伯数字，如 `<h2>1. 子标题</h2>`
   - **三级标题（H3）**：如果原文有编号（如1️⃣2️⃣3️⃣），转换为阿拉伯数字，如 `<h3>1. 小标题</h3>`
   - **编号规则**：所有标题编号统一使用阿拉伯数字（1、2、3...），不要使用中文数字（一、二、三）或圆圈数字（①②③）或emoji数字（1️⃣2️⃣3️⃣）

3. **引用块规则（重要）**：
   - 普通引用：`<blockquote>内容</blockquote>`
   - **高亮引用（重点突出）**：用于重要结论、关键观点、核心洞察，使用 `<blockquote class="highlight">内容</blockquote>`
   - **高亮引用中可使用红色加粗**：`<blockquote class="highlight"><strong style="color:#cf1322;">关键词</strong> 其他内容</blockquote>`
   - 多行引用用 `<br>` 连接
   - **重要**：只有明确的引用内容（如引用某人的话、重要结论）才放入引用块，普通陈述句不要放入引用块

4. **重点突出规则（新增）**：
   - **核心观点/关键结论**：使用红色加粗 `<strong style="color:#cf1322;">重点内容</strong>`
   - **对比强调**：使用高亮引用块包裹对比内容
   - **数字/数据**：用红色突出 `<span style="color:#cf1322;font-weight:bold;">12个错误</span>`
   - **每个章节的主要观点都应该有至少一处红色加粗或高亮引用**
...
9. **行内强调**：
   - 使用 `<span class="emphasis">关键词</span>` 或红色加粗来强调重点词汇
   - **核心概念/关键结论必须用红色加粗突出**
   - 一段话最多使用 1-2 次强调
...
0. **章节标题编号（重要！）**：
   - 所有一级标题（H1）必须使用阿拉伯数字编号，从1开始
   - 例如：`<h1>1. 真实事故</h1>`、`<h1>2. 为什么人类不会</h1>`、`<h1>3. 治理启示</h1>`
   - 三级标题中的emoji数字（1️⃣2️⃣3️⃣）也要转换为阿拉伯数字：`<h3>1. 验证本能</h3>`
   - **不要使用圆圈数字（①②③）、中文数字（一、二、三）或emoji数字**

1. **重点突出（重要！）**：
   - **每个章节的核心观点必须用红色加粗突出**：`<strong style="color:#cf1322;">核心观点</strong>`
   - 重要结论放入高亮引用块：`<blockquote class="highlight">内容</blockquote>`
   - 数字数据用红色突出：`<span style="color:#cf1322;font-weight:bold;">12个</span>`

2. **列表处理**：
   - `- ` 开头的列表：使用 `<ul><li>内容</li></ul>`，多个连续项放同一个 `<ul>` 中
   - `1. ` `2. ` 开头的列表：使用 `<ol><li>内容</li></ol>`，多个连续项放同一个 `<ol>` 中

3. **重点标题 + 列表组合**（如"核心特点"后面跟列表）：
