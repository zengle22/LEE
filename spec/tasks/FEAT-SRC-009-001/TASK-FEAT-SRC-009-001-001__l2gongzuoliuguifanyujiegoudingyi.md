...
基于FEAT-SRC-009-001和ADR-008，建立Feature Delivery L2工作流的正式规范文档，明确：(1)输入规范四字段定义(formal_ssot_id, source_refs, governing_adrs, repo_context)；(2)L3阶段编排顺序Contract→Backend→Frontend→Integration→Evidence Pack；(3)状态机Ready→In Progress→Evidence Pack Produced→Closed；(4)与上游FEAT和下游Evidence Pack的契约接口定义。输出为冻结状态的L2工作流定义文档。

## Acceptance Mapping
- FEAT-SRC-009-001 / AC-001-001: L2工作流定义文档已冻结并通过评审
- FEAT-SRC-009-001 / AC-001-002: 输入规范包含formal_ssot_id, source_refs, governing_adrs, repo_context完整定义
- FEAT-SRC-009-001 / AC-001-003: 明确定义Contract→Backend→Frontend→Integration→Evidence Pack阶段编排顺序
