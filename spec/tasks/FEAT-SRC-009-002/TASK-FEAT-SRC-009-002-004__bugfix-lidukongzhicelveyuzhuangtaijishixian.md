...
基于 FEAT-SRC-009-002 和 ADR-008 的粒度控制要求，实现 Bugfix 粒度控制策略引擎，包括：(1) 默认单 bug 规则 (1 bug -> 1 bugfix workflow instance)；(2) 五同原则 batch 判断逻辑 (同模块、同根因类别、同修复策略、同验证面、同发布窗口)；(3)batch_mode 输入参数验证；(4) 状态机实现 (INIT→TRIAGE→ROOT_CAUSE→FIX_DESIGN→FIX_IMPL→VERIFICATION→EVIDENCE_PACK→MERGE_DECISION→COMPLETED/FAILED)；(5) 状态转换条件和回滚路径定义。
