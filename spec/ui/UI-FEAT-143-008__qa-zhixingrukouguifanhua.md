...
edge_cases:
  auto_complete:
    description: 当用户仅提供 task_ref 时，系统自动补全 plan_ref 和 release_ref
    ui_flow:
    - 从 TASK-001 推导 PLAN-001
    - 从 PLAN-001 推导 REL-001
    - 完整链路校验通过
  parameter_conflict:
    description: 当用户提供的参数与自动推导结果冲突时
    ui_flow:
    - 检测到 TASK-001 不归属 PLAN-999
    - 返回 ERR-CHAIN-003
    - 提示实际归属链路
  concurrent_execution:
    description: 同一 TASK 被多次并发执行时
    ui_flow:
    - 检测到 TASK 正在执行中
    - 分配新 execution_id
    - 继续执行
...
- id: ADR-UI-001
  title: CLI 优先设计
  decision: 采用 CLI 作为主要交互界面
  rationale:
  - QA 执行场景主要在 CI/CD 管道和开发者终端
  - CLI 便于脚本集成和自动化
  - 文本输出便于日志记录和审计追溯
- id: ADR-UI-002
  title: 渐进式校验策略
  decision: 采用 task→plan→release 顺序逐级校验
  rationale:
  - TASK 是最小执行单元，优先验证最有价值
  - 渐进提示帮助用户快速定位问题
  - 避免一次性抛出多个错误造成困惑
- id: ADR-UI-003
  title: 审计前置
