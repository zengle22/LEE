...
扩展 LEE 工作流执行框架的执行器能力，引入 `qwen cli` 作为可选执行器组件，使其能够与 `claude_code`、`codex` 并存，并通过配置在同一套 workflow / runner / executor factory 体系中切换。

## 范围

- `qwen cli` 执行器组件设计与实现
- 执行器工厂对 `qwen` 的注册与实例化支持
...
- 用户可通过 CLI 显式指定 `--executor=qwen` 启动工作流
- 用户可在配置文件中设置 `executor: qwen` 作为默认执行器
- workflow instance 能携带执行器类型并正确传递给 Runner
- `qwen cli` 与 `claude_code`、`codex` 可在同一部署内并存
...
- 配置 schema 支持 `executor_type = "qwen"`
- CLI 参数支持 `--executor=qwen`
- 配置文件支持 `executor: qwen`

### Qwen 执行器工厂实现

- 工厂可基于配置创建 `qwen cli` 执行器实例
