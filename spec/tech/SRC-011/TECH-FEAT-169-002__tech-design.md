...
architecture_decisions:
  ...
  - layer: configuration-resolution
    ...
    reasoning: 复用现有 config_loader.py 架构，扩展 executor 类型识别与验证能力，保持与现有配置系统的向后兼容性
  - layer: validation-layer
    technology: Python Enum + Schema validation
  ...
  - layer: cli-integration
    ...
    reasoning: 复用现有 CLI 框架，添加 --executor 选项，确保 CLI 参数能够正确覆盖配置文件设置
  ...
  - name: ExecutorTypeResolver
    responsibilities: 解析执行器类型配置，实现 CLI > 环境变量 > 配置文件 > 默认设置 的优先级策略，返回最终生效的执行器类型和来源标记
    dependencies:
    - LeeConfig
    - CLI args
...
- name: CLI run command
  ...
  data_flow: CLI args -> ExecutorTypeResolver -> Validation -> workflow_data['executor_config']
- name: ConfigLoader
...
- name: WorkflowRunner
  ...
  data_flow: workflow_data['executor_config'] -> Runner selection -> Executor instantiation
...
- phase: 1
  ...
  - 'AC-002: 单元测试验证优先级规则 (CLI > env > config > default)'
- phase: 2
  title: CLI 集成与配置透传
  tasks:
...
acceptance_mapping:
- ac_id: AC-001
  description: CLI 指定执行器类型
  design_reference: ExecutorTypeResolver CLI 优先级处理
  implementation_phase: 2
- ac_id: AC-002
  description: 配置文件指定执行器类型
  design_reference: ConfigLoader 集成
  implementation_phase: 3
- ac_id: AC-003
  description: 执行器来源优先级判定
  design_reference: 优先级规则 + 配置优先级规则
  implementation_phase: 1
- ac_id: AC-004
  description: 非法执行器配置报错
  design_reference: ConfigValidationErrorHandler + 风险识别
  implementation_phase: 2
exit_codes:
- code: 0
  meaning: 成功
...
- code: 11
  meaning: 执行器不可用
  scenario: 指定的执行器无法加载
frozen_at: '2026-03-12T23:30:00Z'
workflow_instance_id: wf-tech-feat-169-002__tech-design-20260316
approval:
