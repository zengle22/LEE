...
workflow_instance_id: wf-tech-feat-143-012__feat-143-qa-zhixingrukouguifanhua-frozen-technical-20260316
  feature_id: FEAT-143
  title: FEAT-143 QA 执行入口规范化 - Frozen Technical Architecture
  owner: architect
  source_refs:
...
architecture_decisions:
  ...
  - layer: runtime
    ...
    reasoning: 项目现有运行时环境，兼容所有现有依赖
  - layer: data_model
    technology: Python dataclass + typing
    reasoning: 利用 Python 原生 dataclass 定义 ExecutionRequest/ExecutionResult 模型，无需额外依赖，确保类型安全和
      IDE 支持
  - layer: persistence
    technology: aiosqlite (SQLite + WAL mode)
    reasoning: 审计日志需要支持并发写入和快速查询；SQLite 零配置、单文件；WAL 模式提供并发能力；aiosqlite 提供异步支持，已在 pyproject.toml
      中声明
  - layer: ssot_integration
    technology: ArtifactManager + SSOTService
    reasoning: 复用现有 SSOT 基础设施 (src/lee/orchestrator/execution/artifacts/)，通过 ArtifactManager
      读取 front matter 文件，SSOTService 提供校验和查询服务
  - layer: caching
    technology: functools.lru_cache + TTL 装饰器
    reasoning: 减少重复 SSOT 对象查询，60 秒 TTL 平衡一致性与性能，惰性更新避免主动刷新开销
  - layer: cli_framework
    technology: Click (v8.1+)
    reasoning: 项目现有 CLI 框架 (pyproject.toml 已声明)，提供命令注册、参数解析、错误处理能力，与现有 lee 命令集成
  - layer: async_runtime
    technology: asyncio
    reasoning: 支持并发 SSOT 查询、异步审计写入、非阻塞 CLI 响应，Python 3.8 原生支持
  - layer: logging
    technology: Python logging + structlog (可选)
    reasoning: 使用 Python 标准 logging 模块记录审计日志，structlog 作为可选增强提供结构化日志
  core_components:
  - name: EntryRouter
    responsibilities: '执行入口路由的核心接口，负责：
  ...
  - name: BypassBlocker
    responsibilities: '检测和阻断旁路执行请求，识别以下旁路场景：
  ...
  - name: ChainValidator
    responsibilities: '验证 RELEASE->TESTPLAN->TASK 执行路径的完整性，按渐进式顺序逐级校验：
  ...
  - name: AuditLogger
    responsibilities: '记录每次执行请求的审计日志，包含 SSOT 三轴绑定信息：
  ...
  - name: CLI Executor (lee qa execute)
    responsibilities: '实现 lee qa execute 命令，集成 EntryRouter/BypassBlocker/ChainValidator/AuditLogger，
    ...
    options:
    - --task-ref TEXT (必需)
    - --plan-ref TEXT (可选，用于校验)
    - --release-ref TEXT (可选，用于校验)
    - --validate-only (仅校验不执行)
    - --json (JSON 输出)
    - --verbose (详细输出)
    feedback_model:
    - '[1/5] 入口校验'
...
risk_management:
  ...
  - risk_id: R-001
    ...
    degradation_strategy: '若 Registry 不一致或不可用，降级为直接读取 front matter 文件进行校验，
  ...
  - risk_id: R-007
    ...
    - 内存队列批量写入，减少 I/O 次数
    - WAL 模式提供并发写入能力
    degradation_strategy: '若写入延迟过高，减少批量大小或增加后台写入器数量
...
- id: D-001
  ...
  - PostgreSQL + asyncpg (需要额外服务，增加运维复杂度)
  - JSON 文件追加写入 (并发性能差，查询困难)
  - 内存存储 (重启丢失，不适合审计)
  rationale: '轻量、无需额外服务、异步支持、WAL 模式提供并发能力
...
- id: D-003
  ...
  alternatives_considered:
  - 反向校验 (release->plan->task)
  - 并行校验所有节点
  - 仅校验 task 存在性
  rationale: '清晰的错误定位，便于调试，与 ADR-001 交付链语义一致
...
- id: D-004
  ...
  - 永久缓存 (启动时加载)
  - 主动刷新缓存
  rationale: '平衡一致性与性能，减少重复 SSOT 对象查询
...
- id: D-005
  ...
  - 详细日志输出
  - 进度条反馈
  rationale: '与现有 QA Test Set Execute L3 workflow 一致，提供清晰进度反馈
...
- id: D-006
  ...
  - 记录完整调用栈
  - 扁平化键值对
  rationale: '遵循 ADR-001 三轴模型，确保审计记录可追溯到业务/交付/执行全链路
...
- id: D-007
  ...
  - 作为中间件层
  - 作为装饰器
  rationale: '保持关注点分离，EntryRouter 专注于入口校验和路由
...
frozen_declaration:
  ...
  change_policy: '本架构文档已进入 FROZEN 状态，后续变更需走架构变更审批流程：
