...
| **Test Set 维护** | 负责：日常维护、Flaky Test 标记、优先级调整 | 配合：反馈误报和执行问题 |
| **测试执行** | 独立 QA Test Run（可选全量回归） | 强制 Dev Smoke（开发后必须执行） |
| **门禁决策** | 不直接阻塞 merge | Dev Smoke 是强制门禁（blocker） |
| **Smoke 失败处理** | 协助分析：提供测试专家意见 | 负责修复：代码修复和重测 |
| **环境配置** | 提供标准：定义 CI 标准环境配置 | 本地维护：保持本地环境与 CI 一致 |
| **环境一致性** | 维护 CI 标准环境 | 保持本地与 CI 一致 |
| **Flaky Test 治理** | 负责：标记、追踪、修复验证 | 报告：发现并上报 Flaky Test |
